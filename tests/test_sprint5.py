...
from micropki.ocsp import issuer_hashes
...
def _unknown_ocsp_request(issuer_path, serial=0xABCDEF):
    ...
    req = crypto_ocsp.OCSPRequestBuilder().add_certificate_by_hash(
        name_hash,
        key_hash,
        serial,
        hashes.SHA1(),
...
def _post_ocsp(base, body, content_type="application/ocsp-request"):
...
def test_ocsp_signer_certificate_profile():
    with tempfile.TemporaryDirectory() as tmpdir:
        pki, db_path, _, _, _, _, ocsp_cert_path, ocsp_key_path = _prepare_ocsp_workflow(tmpdir)
...
def test_ocsp_good_response_echoes_nonce_and_signature_is_parseable():
    with tempfile.TemporaryDirectory() as tmpdir:
        pki, db_path, inter_cert, _, leaf_cert, _, ocsp_cert, ocsp_key = _prepare_ocsp_workflow(tmpdir)
        server = create_ocsp_server("127.0.0.1", 0, db_path, ocsp_cert, ocsp_key, inter_cert, cache_ttl=120, logger=setup_logger())
        port = server.server_address[1]
        thread = threading.Thread(target=server.serve_forever, daemon=True)
        thread.start()
        try:
            nonce = b"micro-pki-nonce"
            status, headers, body = _post_ocsp(f"http://127.0.0.1:{port}", _ocsp_request(leaf_cert, inter_cert, nonce=nonce))
            assert status == 200
            assert headers["Content-Type"] == "application/ocsp-response"
...
def test_ocsp_request_without_nonce_has_no_nonce_extension():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        server = create_ocsp_server("127.0.0.1", 0, db_path, ocsp_cert, ocsp_key, inter_cert, cache_ttl=60, logger=setup_logger())
        port = server.server_address[1]
        thread = threading.Thread(target=server.serve_forever, daemon=True)
        thread.start()
        try:
            _, _, body = _post_ocsp(f"http://127.0.0.1:{port}", _ocsp_request(leaf_cert, inter_cert))
            response = crypto_ocsp.load_der_ocsp_response(body)
            assert response.response_status == crypto_ocsp.OCSPResponseStatus.SUCCESSFUL
...
def test_ocsp_revoked_and_unknown_statuses():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        thread.start()
        try:
            _, _, revoked_body = _post_ocsp(f"http://127.0.0.1:{port}", _ocsp_request(leaf_cert, inter_cert))
            ...
            _, _, unknown_body = _post_ocsp(f"http://127.0.0.1:{port}", _unknown_ocsp_request(inter_cert))
...
def test_ocsp_malformed_request_and_content_type_errors():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            with pytest.raises(urllib.error.HTTPError) as malformed:
                _post_ocsp(base, b"garbage")
            assert malformed.value.code == 400
            response = crypto_ocsp.load_der_ocsp_response(malformed.value.read())
            assert response.response_status == crypto_ocsp.OCSPResponseStatus.MALFORMED_REQUEST

            with pytest.raises(urllib.error.HTTPError) as bad_ct:
                _post_ocsp(base, b"garbage", content_type="application/octet-stream")
            assert bad_ct.value.code == 400
        finally:
            server.shutdown()
            server.server_close()
            thread.join(timeout=5)
...
def test_openssl_can_inspect_ocsp_signer_certificate():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = subprocess.run(
            ["openssl", "x509", "-in", ocsp_cert, "-text", "-noout"],
            text=True,
            capture_output=True,
        )
...
@pytest.mark.skipif(shutil.which("openssl") is None, reason="OpenSSL CLI is not installed")
def test_openssl_ocsp_client_verifies_good_response_signature():
    with tempfile.TemporaryDirectory() as tmpdir:
        _, db_path, inter_cert, _, leaf_cert, _, ocsp_cert, ocsp_key = _prepare_ocsp_workflow(tmpdir)
        server = create_ocsp_server("127.0.0.1", 0, db_path, ocsp_cert, ocsp_key, inter_cert, cache_ttl=60, logger=setup_logger())
        port = server.server_address[1]
        ...
        try:
            ...
            assert result.returncode == 0, result.stderr
            assert "Response verify OK" in result.stderr
            assert f"{leaf_cert}: good" in result.stdout
        finally:
            server.shutdown()
            server.server_close()
            thread.join(timeout=5)
